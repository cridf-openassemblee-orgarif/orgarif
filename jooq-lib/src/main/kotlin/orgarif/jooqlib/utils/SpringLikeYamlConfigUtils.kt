...
object SpringLikeYamlConfigUtils {
    ...
    // FIXME[fmk][doc] we can debug with this one
    ...
    private fun yamlFilesToPairs(vararg files: InputStream): List<Pair<String, String?>> =
        files
            ...
            .map {
                it.first to
                    // TODO[fmk] write tests (with 2 vars in string, with missing var...)
                    it.second
                        ?.replace("\\$\\{[^}]*}".toRegex()) {
    ...
    private fun flattenConf(map: Map<String, Any>): List<Pair<String, String?>> =
        map.keys.flatMap { key ->
            ...
            when (value) {
                // TODO[fmk] null or not here ?? (if use getValue is useless)
