/*
 * This file is generated by jOOQ.
 */
package orgarif.jooq.generated.tables.records;


import java.time.Instant;
import java.util.UUID;

import javax.annotation.Nonnull;
import javax.annotation.Nullable;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record10;
import org.jooq.Row10;
import org.jooq.impl.UpdatableRecordImpl;

import orgarif.jooq.generated.tables.AppUserTable;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class AppUserRecord extends UpdatableRecordImpl<AppUserRecord> implements Record10<UUID, String, String, String, String, String, Boolean, Instant, String, String[]> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>public.app_user.id</code>.
     */
    public void setId(@Nonnull UUID value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.app_user.id</code>.
     */
    @Nonnull
    public UUID getId() {
        return (UUID) get(0);
    }

    /**
     * Setter for <code>public.app_user.mail</code>.
     */
    public void setMail(@Nonnull String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.app_user.mail</code>.
     */
    @Nonnull
    public String getMail() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.app_user.password</code>.
     */
    public void setPassword(@Nonnull String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.app_user.password</code>.
     */
    @Nonnull
    public String getPassword() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.app_user.username</code>.
     */
    public void setUsername(@Nullable String value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.app_user.username</code>.
     */
    @Nullable
    public String getUsername() {
        return (String) get(3);
    }

    /**
     * Setter for <code>public.app_user.display_name</code>.
     */
    public void setDisplayName(@Nonnull String value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.app_user.display_name</code>.
     */
    @Nonnull
    public String getDisplayName() {
        return (String) get(4);
    }

    /**
     * Setter for <code>public.app_user.language</code>.
     */
    public void setLanguage(@Nonnull String value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.app_user.language</code>.
     */
    @Nonnull
    public String getLanguage() {
        return (String) get(5);
    }

    /**
     * Setter for <code>public.app_user.admin</code>.
     */
    public void setAdmin(@Nonnull Boolean value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.app_user.admin</code>.
     */
    @Nonnull
    public Boolean getAdmin() {
        return (Boolean) get(6);
    }

    /**
     * Setter for <code>public.app_user.signup_date</code>.
     */
    public void setSignupDate(@Nonnull Instant value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.app_user.signup_date</code>.
     */
    @Nonnull
    public Instant getSignupDate() {
        return (Instant) get(7);
    }

    /**
     * Setter for <code>public.app_user.dirty_mail</code>.
     */
    public void setDirtyMail(@Nullable String value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.app_user.dirty_mail</code>.
     */
    @Nullable
    public String getDirtyMail() {
        return (String) get(8);
    }

    /**
     * Setter for <code>public.app_user.former_mails</code>.
     */
    public void setFormerMails(@Nonnull String[] value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.app_user.former_mails</code>.
     */
    @Nonnull
    public String[] getFormerMails() {
        return (String[]) get(9);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    @Nonnull
    public Record1<UUID> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record10 type implementation
    // -------------------------------------------------------------------------

    @Override
    @Nonnull
    public Row10<UUID, String, String, String, String, String, Boolean, Instant, String, String[]> fieldsRow() {
        return (Row10) super.fieldsRow();
    }

    @Override
    @Nonnull
    public Row10<UUID, String, String, String, String, String, Boolean, Instant, String, String[]> valuesRow() {
        return (Row10) super.valuesRow();
    }

    @Override
    @Nonnull
    public Field<UUID> field1() {
        return AppUserTable.APP_USER.ID;
    }

    @Override
    @Nonnull
    public Field<String> field2() {
        return AppUserTable.APP_USER.MAIL;
    }

    @Override
    @Nonnull
    public Field<String> field3() {
        return AppUserTable.APP_USER.PASSWORD;
    }

    @Override
    @Nonnull
    public Field<String> field4() {
        return AppUserTable.APP_USER.USERNAME;
    }

    @Override
    @Nonnull
    public Field<String> field5() {
        return AppUserTable.APP_USER.DISPLAY_NAME;
    }

    @Override
    @Nonnull
    public Field<String> field6() {
        return AppUserTable.APP_USER.LANGUAGE;
    }

    @Override
    @Nonnull
    public Field<Boolean> field7() {
        return AppUserTable.APP_USER.ADMIN;
    }

    @Override
    @Nonnull
    public Field<Instant> field8() {
        return AppUserTable.APP_USER.SIGNUP_DATE;
    }

    @Override
    @Nonnull
    public Field<String> field9() {
        return AppUserTable.APP_USER.DIRTY_MAIL;
    }

    @Override
    @Nonnull
    public Field<String[]> field10() {
        return AppUserTable.APP_USER.FORMER_MAILS;
    }

    @Override
    @Nonnull
    public UUID component1() {
        return getId();
    }

    @Override
    @Nonnull
    public String component2() {
        return getMail();
    }

    @Override
    @Nonnull
    public String component3() {
        return getPassword();
    }

    @Override
    @Nullable
    public String component4() {
        return getUsername();
    }

    @Override
    @Nonnull
    public String component5() {
        return getDisplayName();
    }

    @Override
    @Nonnull
    public String component6() {
        return getLanguage();
    }

    @Override
    @Nonnull
    public Boolean component7() {
        return getAdmin();
    }

    @Override
    @Nonnull
    public Instant component8() {
        return getSignupDate();
    }

    @Override
    @Nullable
    public String component9() {
        return getDirtyMail();
    }

    @Override
    @Nonnull
    public String[] component10() {
        return getFormerMails();
    }

    @Override
    @Nonnull
    public UUID value1() {
        return getId();
    }

    @Override
    @Nonnull
    public String value2() {
        return getMail();
    }

    @Override
    @Nonnull
    public String value3() {
        return getPassword();
    }

    @Override
    @Nullable
    public String value4() {
        return getUsername();
    }

    @Override
    @Nonnull
    public String value5() {
        return getDisplayName();
    }

    @Override
    @Nonnull
    public String value6() {
        return getLanguage();
    }

    @Override
    @Nonnull
    public Boolean value7() {
        return getAdmin();
    }

    @Override
    @Nonnull
    public Instant value8() {
        return getSignupDate();
    }

    @Override
    @Nullable
    public String value9() {
        return getDirtyMail();
    }

    @Override
    @Nonnull
    public String[] value10() {
        return getFormerMails();
    }

    @Override
    @Nonnull
    public AppUserRecord value1(@Nonnull UUID value) {
        setId(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value2(@Nonnull String value) {
        setMail(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value3(@Nonnull String value) {
        setPassword(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value4(@Nullable String value) {
        setUsername(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value5(@Nonnull String value) {
        setDisplayName(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value6(@Nonnull String value) {
        setLanguage(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value7(@Nonnull Boolean value) {
        setAdmin(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value8(@Nonnull Instant value) {
        setSignupDate(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value9(@Nullable String value) {
        setDirtyMail(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord value10(@Nonnull String[] value) {
        setFormerMails(value);
        return this;
    }

    @Override
    @Nonnull
    public AppUserRecord values(@Nonnull UUID value1, @Nonnull String value2, @Nonnull String value3, @Nullable String value4, @Nonnull String value5, @Nonnull String value6, @Nonnull Boolean value7, @Nonnull Instant value8, @Nullable String value9, @Nonnull String[] value10) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached AppUserRecord
     */
    public AppUserRecord() {
        super(AppUserTable.APP_USER);
    }

    /**
     * Create a detached, initialised AppUserRecord
     */
    public AppUserRecord(@Nonnull UUID id, @Nonnull String mail, @Nonnull String password, @Nullable String username, @Nonnull String displayName, @Nonnull String language, @Nonnull Boolean admin, @Nonnull Instant signupDate, @Nullable String dirtyMail, @Nonnull String[] formerMails) {
        super(AppUserTable.APP_USER);

        setId(id);
        setMail(mail);
        setPassword(password);
        setUsername(username);
        setDisplayName(displayName);
        setLanguage(language);
        setAdmin(admin);
        setSignupDate(signupDate);
        setDirtyMail(dirtyMail);
        setFormerMails(formerMails);
    }
}
