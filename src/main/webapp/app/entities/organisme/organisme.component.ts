...
export class OrganismeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.organismeService
        ...
        .subscribe((res: HttpResponse<IOrganisme[]>) => this.paginateOrganismes(res.body, res.headers));
      return;
    }
    this.organismeService
      .query({
  ...
  trackId(index: number, item: IOrganisme) {
    return item.id;
  }

  registerChangeInOrganismes() {
    this.eventSubscriber = this.eventManager.subscribe('organismeListModification', response => this.loadAll());
  }
  ...
  protected paginateOrganismes(data: IOrganisme[], headers: HttpHeaders) {
