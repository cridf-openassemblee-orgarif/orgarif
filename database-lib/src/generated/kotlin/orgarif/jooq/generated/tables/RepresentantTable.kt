...
open class RepresentantTable(
    ...
    val SEARCH_PRENOM: TableField<RepresentantRecord, String?> =
        createField(DSL.name("search_prenom"), SQLDataType.VARCHAR(255).nullable(false), this, "")

    /** The column <code>public.representant.search_nom</code>. */
    val SEARCH_NOM: TableField<RepresentantRecord, String?> =
        createField(DSL.name("search_nom"), SQLDataType.VARCHAR(255).nullable(false), this, "")

    /** The column <code>public.representant.creation_date</code>. */
    val CREATION_DATE: TableField<RepresentantRecord, Instant?> =
        createField(
    ...
    val LAST_MODIFICATION_DATE: TableField<RepresentantRecord, Instant?> =
        createField(
    ...
    constructor(
        child: Table<out Record>,
        key: ForeignKey<out Record, RepresentantRecord>
    ) : this(Internal.createPathAlias(child, key), child, key, REPRESENTANT, null)
    ...
    override fun getReferences(): List<ForeignKey<RepresentantRecord, *>> =
        listOf(REPRESENTANT__REPRESENTANT_ELU_ID_FKEY)

    private lateinit var _elu: EluTable

    /** Get the implicit join path to the <code>public.elu</code> table. */
    fun elu(): EluTable {
        if (!this::_elu.isInitialized) _elu = EluTable(this, REPRESENTANT__REPRESENTANT_ELU_ID_FKEY)
    ...
    override fun `as`(alias: Table<*>): RepresentantTable =
        RepresentantTable(alias.getQualifiedName(), this)
    ...
    /** Rename this table */
    override fun rename(name: Table<*>): RepresentantTable =
        RepresentantTable(name.getQualifiedName(), null)

    // -------------------------------------------------------------------------
    // Row8 type methods
    // -------------------------------------------------------------------------
    override fun fieldsRow():
        Row8<UUID?, UUID?, String?, String?, String?, String?, Instant?, Instant?> =
        super.fieldsRow()
            as Row8<UUID?, UUID?, String?, String?, String?, String?, Instant?, Instant?>

    /** Convenience mapping calling {@link SelectField#convertFrom(Function)}. */
    fun <U> mapping(
        from: (UUID?, UUID?, String?, String?, String?, String?, Instant?, Instant?) -> U
