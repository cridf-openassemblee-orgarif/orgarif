    ...
    private fun doRun(directory: Path?, command: String, vararg params: String): CommandResult {
        val builder =
            ProcessBuilder().apply {
                environment().apply {
                    val pathAdditions =
                        YamlParser.yamlToMap(
                                ShellRunner.javaClass.classLoader.getResourceAsStream(
                                    "tooling-config-${System.getProperty("user.name")}.yaml"))
                            .let { YamlParser.getListByKeyPrefix(it, "pathAdditions") }
                    put("PATH", "${get("PATH")}:${pathAdditions.joinToString(separator = ":")}")
                }
                if (directory != null) {
